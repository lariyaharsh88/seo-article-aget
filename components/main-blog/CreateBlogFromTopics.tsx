...
export function CreateBlogFromTopics() {
  ...
  async function handleCreate() {
    ...
    setCreatedRows([]);
    try {
      const res = await fetch("/api/main-blog/create", {
        method: "POST",
    ...
    <main className="mx-auto max-w-4xl px-4 py-10 md:px-6">
      <section className="rounded-2xl border border-border/80 bg-surface/45 p-5 md:p-8">
        ...
        <p className="mt-3 max-w-2xl font-serif text-sm text-text-secondary md:text-base">
          Enter comma-separated topics. Each topic becomes an unpublished main-domain{" "}
        ...
        <label className="mt-5 block">
          ...
          <textarea
            ...
            placeholder="AI SEO strategy 2026, SaaS onboarding playbook, B2B keyword map"
          />
        </label>
      ...
      {createdRows.length > 0 ? (
        <section className="mt-6 rounded-2xl border border-border/80 bg-surface/45 p-5 md:p-8">
          ...
          <p className="mt-2 font-serif text-sm text-text-secondary">
            These rows are saved with{" "}
            <code className="rounded bg-background/60 px-1 font-mono text-xs">
              siteDomain: main
            </code>{" "}
            and are isolated from education blog listings.
          </p>
